...
function trendPoints(metricId: MetricId, snapshot: TrendSnapshot): readonly number[] {
  ...
  return snapshot.health.flatMap((item) => typeof item.recovery_metrics[0]?.[1] === "number" ? [item.recovery_metrics[0][1]] : []);
}
...
export function DashboardPage() {
  ...
  const trendDetail = expandedId === null ? null : (
    <div className={styles.trendDetail}>
      ...
      {trend.status === "success" && <TrendChart points={trendPoints(expandedId, trend.data)} valueLabel={buildMetricSummaries(view).find((metric) => metric.id === expandedId)?.title ?? "Metric"} emptyLabel="No trend history for this period." />}
    </div>
  );
  return <main className={styles.page}>
    ...
    <section aria-label="Current metrics" className={styles.metrics}>{buildMetricSummaries(view).map((metric) => <MetricCard key={metric.id} title={metric.title} value={metric.value} detail={expandedId === metric.id ? trendDetail : metric.unit} status={metric.status} expanded={expandedId === metric.id} onToggle={() => setExpandedId((current) => current === metric.id ? null : metric.id)} />)}</section>
